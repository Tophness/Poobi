...
def check_direct(hostDict, url, host=None):
    try:
        ...
        # We don't check 'valid' here anymore because we want to catch ALL direct streams
        if '/hls/' in url or url.endswith(direct_check) or '.m3u8' in url:
            return True, url
        return False, url
    except:
        return False, url
...
def process(hostDict, link, host=None, info=None):
    ...
    try:
        ...
        # Check for direct streams first to ensure they aren't filtered by the whitelist
        # as many direct m3u8/mp4 streams are hosted on transient or unknown domains.
        is_dir, dir_link = check_direct(hostDict, link, host=host)
        if is_dir:
            item = make_direct_item(hostDict, dir_link, host=host, info=info)
            if item:
                sources.append(item)
            return sources
